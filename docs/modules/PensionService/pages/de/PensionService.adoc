...
Test
=======
DIES IST EIN PROOF OF CONCEPT FÜR DIE VERSIONSKONTROLLE 

>>>>>>> 198b76269c41be393bad03797f96cc59570344ab
=======
>>>>>>> d4cc616c71145fa2bbab36872fe77dd4abfd6a5d
== Übersicht
...
Benutzer seine sichtbaren Verträge konfigurieren, auf die auch die
...
diese Schritte wird ein Token benötigt, welches der Benutzer zuvor bei den
...
image::Auth_de.png[Image not found,730]
 
Die Authentifizierung erfolgt ebenfalls über den Identity Provider des
Versicherers. Das DRI Portal leitet auf das entsprechende Login-Portal weiter.
Nach einem erfolgreichen Login wird der Kunde zum DRI Portal
weitergeleitet und das Token wird für die spätere Verwendung
gespeichert. Es wird empfohlen auf eine einfache Nutzererfahrung zu achten.
...
https://www.bsi.bund.de/SharedDocs/Downloads/DE/BSI/Publications/CloudComputing/SecurityRecommendationsCloudComputingProviders.pdf +
und +
...
Methode ``requestMasterData(selectedContract, token)`` ab. Die Anfrage
enthält den ausgewählten Vertrag und ein Token, bei Erfolg erhält die DRI 
die Stammdaten und berechnet daraus die Rentenhöhe, die dann an den Kunden zurückgegeben wird.  +
